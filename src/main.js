...
//         stopWebcam(); // Stop camera when switching to cosmic
//     } else {
//         mesh.material = pixelatedMaterial;
//         startWebcam(); // Start camera when switching to pixelated
//     }
// }
...
//     // document.getElementById('cosmic-randomize-btn').addEventListener('click', randomizeCosmicnoremap);
...
//         pixelatedUniforms.u_bgColor.value.setRGB(rgb.r, rgb.g, rgb.b);
//     });

//     document.getElementById('fg-color').addEventListener('input', (e) => {
//         const rgb = hexToRgb(e.target.value);
//         pixelatedUniforms.u_fgColor.value.setRGB(rgb.r, rgb.g, rgb.b);
//     });
...
// function resetPixelatedDefaults() {
...
function initCosmicShader() {
    ...
    const fragmentShader = `
        ...
        vec4 dither(float chance, vec2 uv) {
            return vec4(vec3(step(chance * u_ditherIntensity, dot(sin(uv.xy), cos(uv.yx)))), u_opacity);
        }
...
function setupControls() {
    ...
    const cosmicSliders = [
        ...
        { id: 'frequency', uniform: 'u_frequency', display: 'frequency-value', decimals: 1 },
        { id: 'seed', uniform: 'u_seed', display: 'seed-value', decimals: 1 },
        { id: 'cosmic-opacity', uniform: 'u_opacity', display: 'cosmic-opacity-value', decimals: 2 }
    ];
    ...
    // Pixelated controls
    const pixelatedSliders = [
        { id: 'pixelated-opacity', uniform: 'u_opacity', display: 'pixelated-opacity-value', decimals: 2 }
    ];

    pixelatedSliders.forEach(slider => {
        const input = document.getElementById(slider.id);
        const display = document.getElementById(slider.display);
        input.addEventListener('input', (e) => {
            const value = parseFloat(e.target.value);
            pixelatedUniforms[slider.uniform].value = value;
            display.textContent = value.toFixed(slider.decimals);
        });
    });
...
function resetPixelatedDefaults() {
    ...
    pixelatedUniforms.u_opacity.value = 1.0;
    document.getElementById('pixelated-opacity').value = 1.0;
    document.getElementById('pixelated-opacity-value').textContent = '1.00';
